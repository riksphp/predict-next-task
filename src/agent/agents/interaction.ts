import { Agent, AgentInput, AgentOutput, AgentType } from '../core/types';
import { LlmClient } from '../core/llmClient';
import { interactionPrompt, InteractionResponse } from '../prompts/interaction';
import { isInteractionPlan } from '../schemas/interaction';
import { appendAgentTrace } from '../../features/nextTaskPredictor/data-layer/agentTraceStorage';

export class InteractionAgent implements Agent {
  readonly type: AgentType = 'INTERACTION';
  private llm = new LlmClient();

  async execute(input: AgentInput): Promise<AgentOutput> {
    const startedAtMs = Date.now();
    const json = (await this.llm.generateJson(
      this.type,
      interactionPrompt({ topic: input.goal, context: input.context }),
    )) as InteractionResponse;
    if (!json || !isInteractionPlan(json)) {
      const output: AgentOutput = {
        type: 'CRITIQUE',
        content: { error: 'Invalid interaction plan' },
      };
      const endedAtMs = Date.now();
      await appendAgentTrace({ agent: this.type, input, output, startedAtMs, endedAtMs });
      return output;
    }
    const output: AgentOutput = {
      type: json.control === 'FINAL_ANSWER' ? 'FINAL_ANSWER' : 'PLAN_STEP',
      content: json,
    };
    const endedAtMs = Date.now();
    await appendAgentTrace({ agent: this.type, input, output, startedAtMs, endedAtMs });
    return output;
  }
}
